...
public class DAOMock implements IDAOMock {
	...
	public Product select(int id) {
		Optional<Product> product = productMap.entrySet()
				...
				.map(e -> e.getValue())
				.findFirst();
		return product.orElseThrow(() -> new IllegalArgumentException("no id:" + id));
	}
	
	@Override
	public synchronized void insert(Product product) {
		if(Objects.isNull(product)) {
			throw new NullPointerException("no registered data");
		}
	...
	public synchronized void update(int id, Product product) {
		if(Objects.isNull(product)) {
			throw new NullPointerException("no update data");
		}
		product.setId(id);
		if(count(id) > 0) {
		...
		throw new IllegalArgumentException("no id exsists" + id);
	}
